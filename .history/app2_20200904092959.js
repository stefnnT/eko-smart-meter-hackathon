const mains = document.querySelector('#toggle-mains');
...
const init = _ => {
  ...
  xhr.onload = function() {
    if (xhr.status != 200) { // analyze HTTP status of the response
      ...
      if (result.meterDetails.mainsIn === "on") {
      ...
      if (result.meterDetails.mainsOut === "on") {
      ...
      billingTarriff.textContent = result.meterDetails.tarriffCode.to;
